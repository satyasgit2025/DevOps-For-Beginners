-Day by day traffic is increasing on one of the websites managed by the production support team.
-Therefore, the team has observed a degradation in website performance. 
-Following discussions about this issue, the team has decided to deploy this application on a high availability stack on infra in DC.
-They started the migration last month and it is almost done, as only the LB server configuration is pending. 
-Configure LB server as per the information given below:

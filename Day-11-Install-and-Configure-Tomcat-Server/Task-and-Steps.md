...
1️⃣ Transferred ROOT.war from Jump Host to App Server 2 run below commnad at the Jump Host:
```
...
sudo -i
```
3️⃣ Installed Java & Tomcat:
```
yum install -y java-1.8.0-openjdk
yum install -y tomcat tomcat-webapps tomcat-admin-webapps
```
4️⃣ Configured Tomcat to run on port 6100 for that we have to edit server.xml & update the connector port form 8080 to 6100.
...
```
7️⃣ Verified the Deployment: Check Tomcat status:
```
